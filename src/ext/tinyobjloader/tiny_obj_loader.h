...
static void InitMaterial(material_t &material) {
  ...
  // PBR extension
  material.roughness = 1.f;						// [0, 1] default 1
  material.metallic = 1.f;						// [0, 1] default 1
  material.sheen = 0;							// [0, 1] default 0
  material.clearcoat_thickness = 0;			// [0, 1] default 0
  material.clearcoat_roughness = 0;			// [0, 1] default 0
  material.anisotropy = 0;						// aniso. [0, 1] default 0
  material.anisotropy_rotation = 0;			// anisor. [0, 1] default 0
...
void LoadMtl(std::map<std::string, int> &material_map,
  ...
  while (inStream.peek() != -1) {
	...
	// PBR: metallic
	if (token[0] == 'P' && token[1] == 'm' && isSpace(token[2])) {
		token += 2;
	...
	// PBR: sheen
	if (token[0] == 'P' && token[1] == 's' && isSpace(token[2])) {
		token += 2;
	...
	if (token[0] == 'P' && token[1] == 'c' && isSpace(token[2])) {
		token += 2;
	...
	if ((0 == strncmp(token, "Pcr", 3)) && isSpace(token[3])) {
		token += 4;
	...
	// PBR: anisotropy
	if ((0 == strncmp(token, "aniso", 5)) && isSpace(token[5])) {
		token += 6;
	...
	if ((0 == strncmp(token, "anisor", 6)) && isSpace(token[6])) {
		token += 7;
	...
	// reflection map
	if ((0 == strncmp(token, "refl", 4)) && isSpace(token[4])) {
		token += 5;
		material.reflection_texname = token;
		continue;
	}
	...
	if ((0 == strncmp(token, "map_Pr", 6)) && isSpace(token[6])) {
		token += 7;
	...
	if ((0 == strncmp(token, "map_Pm", 6)) && isSpace(token[6])) {
		token += 7;
	...
	// PBR: sheen texture
	if ((0 == strncmp(token, "map_Ps", 6)) && isSpace(token[6])) {
		token += 7;
